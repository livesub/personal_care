...
/// 조기 종료 건 리스트/갱신 시 즉시 팝업 (실시간 모니터링용).
class _EarlyStopPopupListener extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      ref.read(adminEarlyStopShownIdsProvider.notifier).state =
          Set<int>.from(shown)..addAll(earlyStops.map((e) => e.id));
...
/// 실시간 모니터링 전용 화면. 오늘 일정만 카드 뷰, 탭(전체/서비스 중/시작 전/조기 종료/완료), 1분 자동 갱신, 1초 타이머.
class RealTimeMonitoringScreen extends ConsumerWidget {
        ...
        builder: (context, constraints) {
          ...
          return Column(
            ...
            children: [
              ...
              Expanded(
                child: _buildBody(context, ref, fontScale, i18n, contentPadding, isStaff),
              ),
            ],
          );
        },
  ...
  Widget _buildTopBar(BuildContext context, double fontScale, I18nHelper i18n, double contentPadding) {
    return Padding(
      ...
      child: Row(
        children: [
          Icon(Icons.monitor, color: AppColors.adminActive, size: 22 * fontScale),
          SizedBox(width: 8 * fontScale),
          Text(
            i18n.t('admin_members_nav_monitoring'),
            style: TextStyle(
  ...
  Widget _buildBody(
    ...
    final async = ref.watch(adminMonitorTodayListProvider);
    ...
    return async.when(
      data: (matchings) {
        ...
        Future<void> onEdit(AdminMatchingListItem item) async {
          ...
          ref.invalidate(adminMonitorTodayListProvider);
        }
...
/// 1분마다 오늘 매칭 목록 재조회.
class _AutoRefreshMonitor extends StatefulWidget {
  const _AutoRefreshMonitor({required this.ref, required this.child});

  final WidgetRef ref;
  final Widget child;

  @override
  State<_AutoRefreshMonitor> createState() => _AutoRefreshMonitorState();
}

class _AutoRefreshMonitorState extends State<_AutoRefreshMonitor> {
  Timer? _timer;
  ...
  void initState() {
    ...
    _timer = Timer.periodic(const Duration(minutes: 1), (_) {
      widget.ref.invalidate(adminMonitorTodayListProvider);
    });
  }
...
/// 실시간 모니터링 카드: 서비스 중=연한 녹색 테두리, 중복=#FFE5E5, 지각=빨간 글씨, 위치 경고=⚠️.
